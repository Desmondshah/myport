...
export const LocalNetworkDeviceFinder: React.FC<{ onBack?: () => void }> = ({ onBack }) => {
  const [inputText, setInputText] = React.useState("");
  const [devices, setDevices] = React.useState<ParsedDevice[]>([]);
  const [scanMode, setScanMode] = React.useState<"arp" | "dhcp" | "manual">("arp");
  ...
  const parseLineForDevice = (line: string): ParsedDevice | null => {
    ...
    const arpMatch = line.match(/(\d+\.\d+\.\d+\.\d+).*?([a-fA-F0-9]{2}[:-]){5}[a-fA-F0-9]{2}/);
    if (arpMatch) {
      const ip = arpMatch[1];
    ...
    const dhcpMatch = line.match(/(\d+\.\d+\.\d+\.\d+).*?([a-fA-F0-9]{2}[:-]){5}[a-fA-F0-9]{2}/);
    if (dhcpMatch) {
    ...
    const manualMatch = line.match(/(\d+\.\d+\.\d+\.\d+).*?([a-fA-F0-9]{2}[:-]){5}[a-fA-F0-9]{2}/);
    if (manualMatch) {
    ...
    <div style={containerStyle}>
      ...
      <div style={scanModeStyle}>
        ...
        <div style={modeButtonsStyle}>
          {[
            { id: "arp", label: "ARP_TABLE", icon: "🔍", desc: "Parse ARP cache data" },
            { id: "dhcp", label: "DHCP_LEASES", icon: "📡", desc: "Parse DHCP lease table" },
            { id: "manual", label: "MANUAL_INTEL", icon: "📝", desc: "Custom IP:MAC pairs" }
        ...
        <div style={briefContentStyle}>
          <div style={briefSectionStyle}>
            ...
            <ul style={briefListStyle}>
              ...
              <li>Only use on networks you own or have permission to analyze</li>
            </ul>
          </div>
          ...
          <div style={briefSectionStyle}>
            ...
            <div style={sourcesGridStyle}>
              <div style={sourceItemStyle}>
                ...
                <div>
                  <strong>ARP_CACHE:</strong> arp -a (Windows) | ip neigh (Linux)
                </div>
              </div>
              <div style={sourceItemStyle}>
                <span style={sourceIconStyle}>📡</span>
...
const DeviceCard: React.FC<{ device: ParsedDevice; index: number }> = ({ device, index }) => (
  <div style={{
    ...deviceCardStyle,
    animationDelay: `${index * 0.1}s`
  }}>
    <div style={deviceHeaderStyle}>
      <span style={deviceNumberStyle}>{String(index + 1).padStart(2, '0')}</span>
...
const getSourceColor = (source: string) => {
  switch (source) {
    case "ARP_TABLE": return "#44ff44";
    case "DHCP_LEASE": return "#4444ff";
    case "MANUAL_ENTRY": return "#ffff44";
