...
export const InternetSpeedTest: React.FC<{ onBack?: () => void }> = ({
  ...
  const runTest = async () => {
    ...
    try {
      ...
      try {
        const blob = new Blob([
          crypto.getRandomValues(new Uint8Array(uploadSizeBytes)),
        ]);
              ...
              <div style={advancedPanelStyle}>
                <div style={settingGroupStyle}>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setDownloadSizeBytes(Number(e.target.value))
                    }
                    min="100000"
                    max="10000000"
...
const SpeedGauge: React.FC<{
  ...
  <div
    style={{
      ...gaugeStyle,
      borderColor: color,
      ...(active ? activeGaugeStyle : {}),
    }}
  >
    <div style={gaugeLabelStyle}>{label}</div>
    <div style={gaugeValueStyle}>{value > 0 ? value.toFixed(1) : "--"}</div>
    <div style={gaugeUnitStyle}>{unit}</div>
    {active && (
