...
export const DefensiveBlueTeam: React.FC<{ onBack?: () => void }> = ({
  ...
  const [siemMode, setSiemMode] = React.useState<"setup" | "detection" | "dashboard">("setup");
      ...
      <section style={sectionStyle}>
        ...
        <nav style={missionNavStyle}>
          <button
            ...
            style={{...navBtnStyle, ...(view === "overview" ? activeNavStyle : {})}}
          >
            📋 MISSION BRIEF
          </button>
          <button
            onClick={() => setView("siem")}
            style={{...navBtnStyle, ...(view === "siem" ? activeNavStyle : {})}}
          >
            🔍 SIEM LAB
          </button>
          <button
        ...
        {view === "siem" && <SiemSection mode={siemMode} setMode={setSiemMode} />}
        {view === "ir" && <IncidentResponseSection step={irStep} setStep={setIrStep} />}

        <footer style={pageFooterStyle}>
          <div style={footerContentStyle}>
            <span style={classificationStyle}>🔴 TRAINING CLASSIFICATION: UNCLASSIFIED</span>
            <span style={techStackStyle}>SPLUNK • ELK • VOLATILITY • WIRESHARK</span>
          </div>
        </footer>
      </section>
    </div>
...
// SIEM Section
const SiemSection: React.FC<{ mode: string; setMode: (mode: "setup" | "detection" | "dashboard") => void }> = ({ mode, setMode }) => (
  <div style={siemStyle}>
    <div style={siemNavStyle}>
      <button
        onClick={() => setMode("setup")}
        style={{...siemTabStyle, ...(mode === "setup" ? activeSiemTabStyle : {})}}
      >
        SETUP
      </button>
      <button
        onClick={() => setMode("detection")}
        style={{...siemTabStyle, ...(mode === "detection" ? activeSiemTabStyle : {})}}
      >
        DETECTION
      </button>
      <button
        onClick={() => setMode("dashboard")}
        style={{...siemTabStyle, ...(mode === "dashboard" ? activeSiemTabStyle : {})}}
      >
        DASHBOARD
      </button>
    </div>
      ...
      <div style={siemContentStyle}>
        <div style={terminalStyle}>
          ...
          <div style={terminalBodyStyle}>
            ...
            <div style={commandLineStyle}>
              &gt; Configure data inputs: Windows Event Logs, Sysmon, Apache access
            </div>
            <div style={commandLineStyle}>
        ...
        <div style={setupStepsStyle}>
          ...
          <SetupStep text="Deploy ELK Stack or Splunk instance" completed={true} />
          <SetupStep text="Configure log forwarding (Winlogbeat, Filebeat)" completed={true} />
          <SetupStep text="Ingest Windows Event Logs & Sysmon" completed={true} />
          <SetupStep text="Deploy network monitoring (Zeek, Suricata)" completed={false} />
          <SetupStep text="Configure threat intelligence feeds" completed={false} />
          <SetupStep text="Create detection rules and dashboards" completed={false} />
        </div>
      </div>
    )}
      ...
      <div style={dashboardStyle}>
        ...
        <div style={chartsPlaceholderStyle}>
          <div style={chartBoxStyle}>
            ...
            <div style={chartContentStyle}>Real-time attack visualization would appear here</div>
          </div>
          <div style={chartBoxStyle}>
...
const IncidentResponseSection: React.FC<{ step: number; setStep: (step: number) => void }> = ({ step, setStep }) => {
  const irSteps = [
    "INITIAL DETECTION",
    "CONTAINMENT",
    "EVIDENCE COLLECTION",
  ...
  return (
    <div style={irStyle}>
      ...
      <div style={irContentStyle}>
        {step === 0 && <IRStepContent title="INITIAL DETECTION" content={detectionContent} />}
        {step === 1 && <IRStepContent title="CONTAINMENT" content={containmentContent} />}
        {step === 2 && <IRStepContent title="EVIDENCE COLLECTION" content={evidenceContent} />}
        {step === 3 && <IRStepContent title="ANALYSIS" content={analysisContent} />}
        {step === 4 && <IRStepContent title="ERADICATION" content={eradicationContent} />}
        {step === 5 && <IRStepContent title="RECOVERY" content={recoveryContent} />}
        {step === 6 && <IRStepContent title="LESSONS LEARNED" content={lessonsContent} />}
      </div>
    </div>
  );
};
...
// Helper Components
const ObjectiveItem: React.FC<{ icon: string; title: string; description: string; status: string }> = ({
  ...
  <div style={objectiveItemStyle}>
    ...
    <div style={{...objectiveStatusStyle, 
      color: status === 'ACTIVE' ? '#00ff00' : 
             status === 'READY' ? '#00d9ff' : '#ffff00'
    }}>
      {status}
    </div>
  </div>
...
const CapabilityCard: React.FC<{ title: string; description: string; technologies: string[]; color: string }> = ({
  ...
  <div style={{...capabilityCardStyle, borderColor: color}}>
    ...
    <div style={techTagsStyle}>
      {technologies.map((tech, i) => (
        <span key={i} style={{...techTagStyle, borderColor: color}}>{tech}</span>
      ))}
    </div>
  </div>
);

const SetupStep: React.FC<{ text: string; completed: boolean }> = ({ text, completed }) => (
  <div style={setupStepStyle}>
    <div style={{...stepIndicatorStyle, background: completed ? '#00ff00' : '#333'}}>
      {completed ? '✓' : '○'}
    </div>
    <span style={{...stepTextStyle, color: completed ? '#fff' : '#666'}}>{text}</span>
  </div>
);

const DetectionRule: React.FC<{ name: string; description: string; severity: string; triggered: boolean; events: number }> = ({
  ...
  <div style={{...ruleCardStyle, borderColor: getSeverityColor(severity)}}>
    <div style={ruleHeaderStyle}>
      ...
      <span style={{...ruleSeverityStyle, color: getSeverityColor(severity)}}>{severity}</span>
    </div>
...
const AlertEntry: React.FC<{ timestamp: string; severity: string; rule: string; message: string }> = ({
  timestamp, severity, rule, message
}) => (
  <div style={alertEntryStyle}>
    <span style={alertTimeStyle}>{timestamp}</span>
    <span style={{...alertSeverityStyle, color: getSeverityColor(severity)}}>{severity}</span>
...
const IRStepContent: React.FC<{ title: string; content: string[] }> = ({ title, content }) => (
