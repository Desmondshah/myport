...
export const RealTimeSystemHealthMonitor: React.FC<{ onBack: () => void }> = ({
  ...
  const [viewMode, setViewMode] = React.useState<"dashboard" | "charts" | "logs">("dashboard");
      ...
      <section style={sectionStyle}>
        ...
        <div style={statusBarStyle}>
          ...
          <div style={statusItemStyle}>
            ...
            <div style={statusIndicatorStyle}>
              <div style={{...statusDotStyle, background: isOnline ? '#00ff00' : '#ff0000'}} />
              <span style={statusValueStyle}>{isOnline ? 'ONLINE' : 'OFFLINE'}</span>
            </div>
          </div>
          <div style={statusItemStyle}>
        ...
        <nav style={navStyle}>
          <button
            ...
            style={{...navBtnStyle, ...(viewMode === "dashboard" ? activeNavStyle : {})}}
          >
            DASHBOARD
          </button>
          <button
            onClick={() => setViewMode("charts")}
            style={{...navBtnStyle, ...(viewMode === "charts" ? activeNavStyle : {})}}
          >
            CHARTS
          </button>
          <button
            onClick={() => setViewMode("logs")}
            style={{...navBtnStyle, ...(viewMode === "logs" ? activeNavStyle : {})}}
          >
            LOGS
          </button>
        </nav>
              ...
              <div style={controlsStyle}>
                ...
                <div style={controlGroupStyle}>
                  ...
                  <select
                    ...
                    style={selectStyle}
                  >
                    <option value={Date.now() - 5 * 60 * 1000}>Last 5 minutes</option>
                    <option value={Date.now() - 15 * 60 * 1000}>Last 15 minutes</option>
                    <option value={Date.now() - 60 * 60 * 1000}>Last hour</option>
                    <option value={Date.now() - 24 * 60 * 60 * 1000}>Last 24 hours</option>
                  </select>
                </div>
              </div>
            </div>
          ...
          <div style={logsStyle}>
            <div style={logTerminalStyle}>
              <div style={logHeaderStyle}>
                ...
                <span style={logCountStyle}>{samples?.length || 0} ENTRIES</span>
              </div>
              <div style={logBodyStyle}>
                {samples?.slice(-20).reverse().map((sample, i) => (
                  <div key={i} style={logEntryStyle}>
                    <span style={timestampStyle}>
                      {new Date(sample.ts).toLocaleTimeString()}
                    </span>
                    <span style={logDataStyle}>
                      CPU: {sample.cpuScore?.toLocaleString() || 'N/A'} | 
                      RTT: {sample.netRtt || 'N/A'}ms | 
                      Ping: {sample.netPingMs?.toFixed(0) || 'N/A'}ms
                    </span>
                  </div>
                )) || (
